...
/**
 * 蜘蛛统计（手动根目录建立 robotslogs.txt 文档）
 * @version 1.0
 * @package Vtrois
 */  
...
function addLink() {
    ...
    var pagelink = "<br /><br /> 著作权归作者所有。<br>商业转载请联系作者获得授权，非商业转载请注明出处。<br>来源：Vasilis's Blog<br>作者：Vasilis<br>网址：www.aiwangxu.top<br></a>";
...
function colorCloud($text) {
    $text = preg_replace_callback('/<a (.+?)>/', 'colorCloudCallback', $text);
    return $text;
}
function colorCloudCallback($matches) {
    ...
    //随机颜色
    $colors = array('337ab7', '1ac756', 'ff5e5c', 'ffbb50', '5bc0de', 'A349A4', '567E95', 'FF6600', '00ABA9', '90BF11');
    $color = $colors[dechex(rand(0, sizeof($colors) - 1))];
    
    //加入类 sidebar-tags
    $pattern = '/(class=)([\'\"])(.*?)(\\2)/';
    $text = preg_replace($pattern, '$1$2$3 sidebar-tags$2', $text);
    
    //加入随机的背景颜色
    $pattern = '/(style=)([\'\"])(.*?)(\\2)/';
    $text = preg_replace($pattern, "$1$2background-color:#{$color};$2", $text); 
